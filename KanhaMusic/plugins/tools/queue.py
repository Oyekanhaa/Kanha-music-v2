...
from KanhaMusic.utils import KanhaBin, get_channeplayCB, seconds_to_min
...
async def get_queue(client, message: Message, _):
    ...
    cap = _["queue_8"].format(kanha.mention, title, typo, user, send)
    upl = (
...
@kanha.on_callback_query(filters.regex("GetTimer") & ~BANNED_USERS)
...
@kanha.on_callback_query(filters.regex("GetQueued") & ~BANNED_USERS)
@languageCB
...
@kanha.on_callback_query(filters.regex("queue_back_timer") & ~BANNED_USERS)
@languageCB

...
async def executor(client: kanha, message: Message):
...
@kanha.on_callback_query(filters.regex(r"runtime"))
async def runtime_func_cq(_, cq):
    runtime = cq.data.split(None, 1)[1]
    await cq.answer(runtime, show_alert=True)


@kanha.on_callback_query(filters.regex("forceclose"))
...
async def maintenance_executor(client: kanha, message: Message):
    if len(message.command) < 2:

...
async def reload_admin_cache(client, message: Message, _):
    try:
        ...
        async for user in kanha.get_chat_members(
...
@kanha.on_message(filters.command(["reboot"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def restartbot(client, message: Message, _):
    mystic = await message.reply_text(_["reload_4"].format(kanha.mention))
    ...
    if chat_id:
        try:
            got = await kanha.get_chat(chat_id)
        except:
            pass
        ...
        try:
            db[chat_id] = []
            await Kanha.stop_stream_force(chat_id)
        except:
            pass
    return await mystic.edit_text(_["reload_5"].format(kanha.mention))


@kanha.on_callback_query(filters.regex("close") & ~BANNED_USERS)
...
@kanha.on_callback_query(filters.regex("stop_downloading") & ~BANNED_USERS)
@ActualAdminCB

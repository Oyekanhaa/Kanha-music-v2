...
async def settings_mar(client, message: Message, _):
    ...
    await message.reply_text(
        _["setting_1"].format(kanha.mention, message.chat.id, message.chat.title),
        reply_markup=InlineKeyboardMarkup(buttons),
    )


@kanha.on_callback_query(filters.regex("settings_helper") & ~BANNED_USERS)
@languageCB
...
@kanha.on_callback_query(filters.regex("settingsback_helper") & ~BANNED_USERS)
@languageCB
async def settings_back_markup(client, CallbackQuery: CallbackQuery, _):
    ...
    if CallbackQuery.message.chat.type == ChatType.PRIVATE:
        await kanha.resolve_peer(OWNER_ID)
        OWNER = OWNER_ID
        buttons = private_panel(_)
        return await CallbackQuery.edit_message_text(
            _["start_2"].format(CallbackQuery.from_user.mention, kanha.mention),
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    else:
        buttons = setting_markup(_)
...
@kanha.on_message(filters.command(["repo", "source"]) & filters.private)
async def send_repo(_, message: Message):
    await message.reply_video(
        ...
        caption=(
            ...
            "🧡 ᴄʀᴇᴅɪᴛꜱ : <a href='https://t.me/aboutkanha'>@aboutkanha</a>"
        ),
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("🎵 Music + Management Bot Repo", url="https://github.com/Kanha/KanhaMusic")],
                [InlineKeyboardButton("🎧 Only Music Bot Repo", url="https://github.com/Kanha/KanhaMusic")]
            ]
        ),
...
@kanha.on_callback_query(filters.regex("FERRARIUDTI") & ~BANNED_USERS)
@ActualAdminCB
...
@kanha.on_callback_query(filters.regex(pattern=r"^(AUTH|AUTHLIST)$") & ~BANNED_USERS)
@ActualAdminCB
    ...
    if command == "AUTHLIST":
        ...
        if not _authusers:
            ...
            for note in _authusers:
                ...
                try:
                    user = await kanha.get_users(user_id)
...
@kanha.on_callback_query(filters.regex("VOMODECHANGE") & ~BANNED_USERS)
@ActualAdminCB

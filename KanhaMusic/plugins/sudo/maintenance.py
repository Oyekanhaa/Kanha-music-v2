...
@kanha.on_message(filters.command(["maintenance"]) & SUDOERS)
async def maintenance(client, message: Message):
    ...
    if state == "enable":
        if await is_maintenance() is False:
            ...
            await message.reply_text(_["maint_2"].format(kanha.mention))
    elif state == "disable":
        if await is_maintenance() is False:
            await maintenance_off()
            await message.reply_text(_["maint_3"].format(kanha.mention))
        else:

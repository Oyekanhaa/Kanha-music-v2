...
@kanha.on_message(filters.command(["gban", "globalban"]) & SUDOERS)
@language
async def global_ban(client, message: Message, _):
    ...
    for chat_id in served_chats:
        try:
            await kanha.ban_chat_member(chat_id, user.id)
            number_of_chats += 1
...
@kanha.on_message(filters.command(["ungban"]) & SUDOERS)
@language
async def global_un(client, message: Message, _):
    ...
    for chat_id in served_chats:
        try:
            await kanha.unban_chat_member(chat_id, user.id)
            number_of_chats += 1
        except FloodWait as fw:
            await asyncio.sleep(int(fw.value))
        except:
...
@kanha.on_message(filters.command(["gbannedusers", "gbanlist"]) & SUDOERS)
@language

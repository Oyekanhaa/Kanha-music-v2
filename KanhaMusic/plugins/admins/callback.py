...
@kanha.on_callback_query(filters.regex("ADMIN") & ~BANNED_USERS)
@languageCB
    ...
    if command == "Pause":
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await Kanha.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Kanha.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            run = await CallbackQuery.message.reply_photo(
                ...
                caption=_["stream_1"].format(
                    f"https://t.me/{kanha.username}?start=info_{videoid}",
                    title[:23],
                    duration,
                    user,
                ),
            ...
            try:
                ...
                image = None
            try:
                await Kanha.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await Kanha.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Kanha.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
